...
**Escape hatch:** if a misconfigured (too-high) harvest threshold would otherwise stall the protocol forever — bribes for an epoch can only be claimed once — `forceCloseEpoch()` closes out the epoch without any token clearing its threshold. Already-claimed balances stay in the contract and roll into the next epoch's snapshot.

Governance can call it at any time. **Anyone** can call it once the epoch is more than `FORCE_CLOSE_DELAY` (2 weeks) past its natural end, because every state that needs this hatch is otherwise a permanent freeze if the governance key is lost: an epoch voted through auto-select with `gauges` never configured (claim and harvest both refuse), or one where nothing was harvested. The permissionless path additionally requires the epoch to have been voted — an unvoted epoch has nothing stranded in it, and that requirement is also what stops the path being called in a loop to burn through epoch numbers. `forceCloseStatus()` reports when it opens. Value only ever moves into `carriedOver`, where the next clearing taxes it normally, so a late close costs a delay and nothing else.
