...
import { h2, h3, p, list, steps, callout, table, cards, kv } from './types';
    ...
    pages: [
      {
        ...
        blocks: [
          p('BynD is a **Convex-style boost aggregator** for veMEZO on Mezo, the Bitcoin-collateralized DeFi chain. It pools everyone\'s locked veMEZO into one shared position, votes that combined power toward the highest-yielding veBTC gauges every epoch, and mints each depositor a liquid, tradeable token — **veBYND** — 1:1 against what they locked.'),
          callout('info', 'If you\'ve used Convex on Ethereum, the mental model transfers directly: BynD is to veMEZO/Mezo what Convex is to veCRV/Curve.'),
          h2('why-pool', 'Why pool veMEZO at all?'),
          p('Locking MEZO into a veMEZO NFT gives you voting power over which gauges receive emissions — but a single small lock rarely moves the needle on its own, and the lock itself is illiquid until it expires. Pooling changes both problems at once:'),
          cards([
            { icon: Layers, title: 'Bigger boost, together', body: 'Combined voting power reaches gauge emissions and bribes that a solo lock never could on its own.' },
            { icon: ArrowRightLeft, title: 'Liquidity without waiting', body: 'veBYND is freely tradeable the moment it\'s minted — no need to wait out the lock to get value out.' },
            { icon: Vote, title: 'Voting is automated', body: 'optimiseAndVote() routes the pool toward the best-yielding veBTC gauges every epoch — nobody has to vote manually.' },
            { icon: Wrench, title: 'Upkeep runs itself', body: 'Every maintenance call — voting, harvesting, extending locks — is permissionless and pays its caller a bounty.' },
          ]),
          h2('how-it-fits', 'How the pieces fit together'),
          p('Four contracts do the work: **ByNdVault** holds the veMEZO NFTs and mints veBYND, **ByNdVoter** routes votes and harvests rewards, **ByNdStaking** pays those rewards out to anyone staking veBYND, and **VeBYND** is the liquid ERC-20 itself. The next few pages walk through each one — start with Quickstart if you just want to try it, or Protocol Overview if you want the full mental model first.'),
        ],
      },
      {
        slug: 'quickstart',
    ...
    pages: [
      ...
      },
      {
        ...
        blocks: [
          ...
          p('Once multiple veMEZO NFTs are deposited into `ByNdVault`, their voting power isn\'t tracked separately — it\'s summed into one number (`totalVotingPower`) that `ByNdVoter` casts as a single block every epoch. That aggregation is the entire thesis: gauges and bribe markets respond to how much power shows up at once, and a pooled block reaches influence that fragmented, individually-managed locks rarely do.'),
          callout('info', 'Deposited veMEZO NFTs are also kept from expiring automatically. `ByNdVault.extendLocks(tokenIds[])` is permissionless and batched (up to 200 per call) — any keeper can call it to push locks back out to the max 4-year duration, tracked via `lastExtendTimestamp` and discovered via the paginated `tokensNeedingExtend(offset, limit)` view.'),
        ],
      },
      {
        slug: 'vebynd',
        ...
        blocks: [
          ...
          p('To exit a position, trade veBYND for MEZO on the veBYND/MEZO liquidity pool, seeded at launch. Price will track the pool\'s depth and the market\'s view of BynD\'s yield, similar to how cvxCRV or other Convex-style receipt tokens trade against their underlying.'),
        ],
      },
      {
        slug: 'vault-staking',
        ...
        blocks: [
          ...
          table(
            ...
            [
              ['`deposit(tokenId)`', 'Takes custody of a veMEZO NFT and mints veBYND 1:1 to the caller.'],
              ['`extendLocks(tokenIds[])`', 'Batched (max 200/call), permissionless. Extends locks back toward the 4-year max so the pool\'s voting power never decays to zero.'],
              ['`claimRebases(tokenIds[])`', 'Permissionless, no epoch gate. Compounds any MEZO rebase accrued on the locked NFTs directly back into the lock — nothing leaves the vault.'],
              ['`totalVotingPower()` / `totalLockedMEZO()`', 'View functions — the pool\'s current aggregated numbers.'],
              ['`getUserTokens(user)` / `getAllTokenIds()`', 'Enumerate deposited tokens, since deposits are custodied rather than burned.'],
            ],
          ),
          ...
          table(
            ...
            [
              ...
              ['`claimAll()`', 'Claims every registered reward token in one transaction — the only claim entrypoint in v2.'],
      ...
      },
      {
        ...
        blocks: [
          p('A **gauge** is where votes get directed to earn emissions — in BynD\'s case, veBTC gauges on Mezo. `ByNdVoter.optimiseAndVote()` casts the vault\'s entire pooled voting power toward whichever gauge (or set of gauges) is currently offering the best yield, auto-selecting a live gauge if governance hasn\'t configured one explicitly.'),
          h2('bribes', 'Bribes'),
      ...
      },
      {
        ...
        blocks: [
          p('BynD runs on Mezo\'s weekly epoch cycle. Some vault functions are gated to run once per epoch; others are open at any time. Anyone — not just a designated operator — can call any of them, which is what "keeper" means here: it\'s a role anyone can fill, not a whitelisted address.'),
          table(
            ...
            [
              ...
              ['`extendLocks(tokenIds[])`', 'No epoch gate, batched', 'Keeps locked veMEZO from decaying toward expiry'],
              ['`claimRebases(tokenIds[])`', 'No epoch gate', 'Compounds accrued MEZO rebases back into locks'],
            ],
          ),
    ...
    pages: [
      ...
      },
      {
        ...
        blocks: [
          ...
          steps([
            { title: 'Anyone can call it', body: 'Open the Keeper page and use **Cast system votes**. There\'s no special permission required — the contract itself has no time window gate, per its own doc comments.' },
      ...
      },
      {
        ...
        blocks: [
          ...
          steps([
            ...
            { title: 'Extend locks', body: 'Run `extendLocks(tokenIds[])` on any NFTs flagged by `tokensNeedingExtend()` so the pool\'s voting power keeps compounding instead of decaying toward expiry.' },
            { title: 'Claim rebases', body: 'Not epoch-gated — run `claimRebases(tokenIds[])` whenever convenient to compound accrued MEZO rebases back into the vault\'s locks.' },
          ]),
    ...
    pages: [
      ...
      },
      {
        ...
        blocks: [
          kv([
            ...
            { label: 'extendLocks batch cap', value: '200 token IDs per call' },
          ]),
      ...
      },
      {
        ...
        blocks: [
          ...
          p('Nobody exclusively — every maintenance call (voting, harvesting, extending locks, claiming rebases) is permissionless. Anyone can call them from the Keeper page, and `harvestAndDistribute()` pays whoever calls it a 1% bounty.'),
        ],
      },
      {
        slug: 'glossary',
        ...
        blocks: [
          table(
            ...
            [
              ...
              ['Epoch', 'Mezo\'s weekly accounting period. Some BynD functions (`harvestAndDistribute`, `markLocksExtended`) are gated to run once per epoch.'],
