    ...
    pages: [
      ...
      },
      {
        ...
        blocks: [
          ...
          callout('info', 'Deposited veMEZO NFTs are also kept from expiring automatically. `ByNdVault.extendLocks()` is permissionless and takes no arguments — any keeper can call it any time to push the pool\'s lock(s) back out to the max 4-year duration. In the rare case a deposit can\'t be merged (e.g. it already voted elsewhere this epoch, or is itself an unvested grant NFT), it\'s kept as a separately-managed straggler until it becomes mergeable in a later epoch.'),
        ],
      },
      {
        slug: 'vebynd',
